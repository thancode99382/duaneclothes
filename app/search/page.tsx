...
export default function SearchPage() {
  ...
  return (
    <main className="min-h-screen flex flex-col text-white font-sans bg-[#031f41] relative overflow-hidden">
      {/* Dynamic Background Noise */}
      <div
        className="fixed inset-0 opacity-[0.12] pointer-events-none z-0 mix-blend-multiply"
        style={{
          backgroundImage:
      ...
      <div className="w-full max-w-[1400px] mx-auto px-6 py-12 lg:py-20 relative z-10 flex-1 flex flex-col">
        ...
        <div className="w-full mb-16 lg:mb-24 flex flex-col items-center">
          <div
            className={`w-full max-w-4xl transition-all duration-500 ease-[cubic-bezier(0.33,1,0.68,1)] ${isFocused ? "scale-[1.02]" : "scale-100"}`}
          >
            <div className="relative group">
              <div
                className={`absolute -inset-1 bg-gradient-to-r from-[#d91a1a] via-black to-[#d91a1a] opacity-20 blur-lg transition-all duration-500 ${isFocused ? "opacity-40 blur-xl scale-105" : "group-hover:opacity-30"}`}
              ></div>
              <div
                className={`relative bg-white flex items-center border-4 border-black p-2 md:p-4 shadow-[12px_12px_0px_0px_rgba(217,26,26,1)] transition-all duration-300 ${isFocused ? "shadow-[16px_16px_0px_0px_rgba(217,26,26,1)] translate-x-[-4px] translate-y-[-4px]" : ""}`}
              >
                <SearchIcon
                  className={`h-8 w-8 md:h-10 md:w-10 ml-4 transition-colors duration-300 ${isFocused ? "text-[#d91a1a]" : "text-neutral-400"}`}
                />
                <input
                  type="text"
                ...
                {query && (
                  ...
                  >
                    <X
                      className="h-6 w-6 md:h-8 md:w-8 text-black group-hover/clear:text-[#d91a1a] transition-colors"
                      strokeWidth={3}
                    />
                  </button>
                )}
        ...
        {/* Results Area */}
        <div
          className={`w-full relative transition-opacity duration-500 ${mounted ? "opacity-100" : "opacity-0"}`}
        >
          <div className="flex items-end justify-between mb-10 border-b-[3px] border-black pb-4">
            <h2 className="text-3xl md:text-5xl font-black uppercase text-white tracking-tighter flex items-center gap-4">
              {query ? "KẾT QUẢ" : "THỊNH HÀNH"}
              {query && (
                <span className="text-[#d91a1a] truncate max-w-[200px] md:max-w-[400px]">
                  "{query}"
                </span>
              )}
            </h2>
              ...
              {filteredProducts.map((product, index) => (
                ...
                >
                  ...
                  <div className="flex flex-col flex-1 pl-3 border-l-[3px] border-transparent group-hover:border-[#d91a1a] transition-all duration-300">
                    <h3 className="font-black text-xl md:text-2xl uppercase tracking-tight leading-none text-white mb-3 pr-4 group-hover:text-[#d91a1a] transition-colors">
                      {product.name}
                    </h3>
                    <p className="font-black text-lg md:text-xl text-white mt-auto">
            ...
            <div className="w-full flex flex-col items-center justify-center py-20 md:py-32 text-center animate-in fade-in duration-500">
              <div className="relative mb-10 group">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <X
                    className="h-16 w-16 md:h-24 md:w-24 text-[#d91a1a] transition-transform duration-500 group-hover:scale-110 group-hover:rotate-90"
                    strokeWidth={4}
                  />
                </div>
              </div>
              <h2 className="text-4xl md:text-6xl lg:text-8xl font-black uppercase tracking-tighter text-black mb-6 leading-none">
                KHÔNG TÌM THẤY GÌ
              </h2>
              <p className="text-lg md:text-xl text-neutral-500 font-medium max-w-lg mx-auto">
                Chúng tôi không thể tìm thấy bất cứ thứ gì khớp với "{query}".
                Hãy thử kiểm tra lỗi chính tả hoặc sử dụng các từ khóa khác.
              </p>
              <button
                onClick={() => setQuery("")}
